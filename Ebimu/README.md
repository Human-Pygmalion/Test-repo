...
- **축 화살표** — 보드 윗면에서 나가고 보드와 같이 돕니다 (매뉴얼 4-1 배치).
  평면 두 장을 90° 로 교차시켜 어느 각도에서 봐도 납작해 보이지 않습니다
  - `X+ ROLL` 빨강 (패드 열 방향) · `Y+ PITCH` 초록 · `Z+ YAW` 파랑
  - 회색 원과 `+` 는 그 축의 회전 + 방향입니다
- **바닥 격자와 그림자** — 보드가 얼마나 기울고 떠 있는지 가늠하는 기준
- **`g 계산` 회색 화살표** — 자세에서 계산한 중력벡터를 **받은 값 그대로** 그립니다
- **`g 가속도계` 주황 화살표** — 가속도계가 잰 방향. 자세 계산과 무관한 독립적인 값입니다
  - 정지 상태에서 **둘이 겹치면 정상**입니다
  - 벌어지면 부착 방향이나 오일러 순서가 틀린 것입니다 (패널의 `가속도차` 와 같은 이야기)
  - `<soa1>`(중력 포함) 일 때만 뜻이 있습니다. `<soa2>`/`<soa3>` 은 중력을 뺀 값이라
    정지 상태에서 주황 화살표가 제멋대로 움직입니다
